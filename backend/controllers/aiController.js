...
const generateSummary = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: "Please add meeting notes or upload a file first.",
      });
    }
...
`;
    ...
    const result = await generateMeetingSummary(aiInput);

    console.log("AI RESULT:", result);

    meeting.summary = result.summary || "Summary unavailable";
    ...
    meeting.deadlines = result.deadlines || [];
...
`;
    ...
    const answer = await askMeetingQuestion(context, question);
